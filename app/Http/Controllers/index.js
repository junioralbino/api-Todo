...
const randomNumbers = [10, 30, 15, 25, 50, 40, 5] //numero  % 2 e par numero % 1 impar

const numeroImpar  = randomNumbers.filter(randomNumber => randomNumber % 2 === 1 )
...
const numberCrazy =  crazyNumbers.reduce((accumulator, crazyNumber) => {
    if(accumulator < 501){
        accumulator += 1
    }
...
const numbers = [5, 7, 3]
const number2 = numbers.map(item => item ** 2)
...
const moviesfilmes = tarantinoMovies.filter(tarantinoMovie => tarantinoMovie.release < 2000)

console.log(moviesfilmes)
...
const nomeFilme = tvShows.map(tvShow => {name})
